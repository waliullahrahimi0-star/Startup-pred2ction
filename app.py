...
with st.sidebar:
    ...
    selected_category = st.selectbox(
        ...
        help="Main area the company operates in",
    )
    ...
    selected_state = st.selectbox(
        "State or area",
        options=state_opts,
        help="State, province, or area within the country",
    )
    ...
    selected_region = st.selectbox(
        "Region",
        options=region_opts,
        help="Broader geographic region of the company",
    )
    ...
    selected_city = st.selectbox(
        "City",
        options=city_opts,
        help="City where the company is headquartered",
    )
    ...
    funding_rounds = st.slider(
        ...
        max_value=20,
        value=2,
        help="Number of times the company received investment",
    )
    ...
    founded_year = st.number_input(
        ...
        max_value=2026,
        value=2010,
        help="When the company was started",
    )
    ...
    first_year = st.number_input(
        ...
        min_value=1980,
        max_value=2026,
        value=max(2011, int(founded_year)),
        help="Year of the first investment",
    )
    ...
    last_year = st.number_input(
        ...
        min_value=1980,
        max_value=2026,
        value=max(2014, int(first_year)),
        help="Most recent funding activity",
    )
...
if predict_button:
    ...
    # Derived duration features — soft clamped to zero if dates are inconsistent
    time_to_first = max(0, (int(first_year) - int(founded_year)) * 365)
    time_between  = max(0, (int(last_year)  - int(first_year))  * 365)
    ...
    display_country = selected_country if selected_country != "(not specified)" else "Not specified"
    display_state   = selected_state   if selected_state   != "(not specified)" else "Not specified"
    display_region  = selected_region  if selected_region  != "(not specified)" else "Not specified"
    display_city    = selected_city    if selected_city    != "(not specified)" else "Not specified"

    summary_rows = [
        ("Business sector",          selected_category),
        ("Country",                  display_country),
        ("State or area",            display_state),
        ("Region",                   display_region),
        ("City",                     display_city),
        ("Total funding (USD)",      f"${funding_total:,.0f}"),
        ("Number of funding rounds", str(int(funding_rounds))),
        ("Year founded",             str(int(founded_year))),
        ("First funding year",       str(int(first_year))),
        ("Latest funding year",      str(int(last_year))),
    ]

    rows_html = "".join(
        f"<tr><td>{feature}</td><td>{value}</td></tr>"
        for feature, value in summary_rows
    )
...
This tool uses a tuned Random Forest classifier trained on historical startup data. The model was selected after comparing it against Logistic Regression and Decision Tree baselines.

**Training data**

Only startups with a confirmed outcome — either acquired, IPO, or closed — were used for training. Companies still in operation were excluded, as their final outcome remains unknown.

**Features used**

The model draws on funding amounts, number of rounds, business sector, geography, founding year, and the timing between funding events.

**Limitations**

No model can predict startup outcomes with certainty. This tool is intended as a decision-support aid and should not be used as the sole basis for investment decisions. Results may reflect biases present in the original dataset, particularly towards US-based technology companies.
""")
